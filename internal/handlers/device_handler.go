...
func (h *DeviceHandler) Create(c *gin.Context) {
    ...
    if err := c.ShouldBindJSON(&req); err != nil { apperror.JSONError(c, http.StatusBadRequest, "validation_error", "invalid request payload", err.Error()); return }
    ...
    if err != nil { httpError(c, err); return }
    d.ID = id
...
func (h *DeviceHandler) Update(c *gin.Context) {
    ...
    if req.CreatedAt != nil { created = models.NewFormattedTime(*req.CreatedAt) } else { ex, err := h.svc.Get(c, id); if err != nil { httpError(c, err); return }; created = ex.CreatedAt }
    d := models.Device{Name: req.Name, Brand: req.Brand, State: models.State(req.State), CreatedAt: created}
    if err := h.svc.Update(c, id, &d); err != nil { httpError(c, err); return }
...
func (h *DeviceHandler) Patch(c *gin.Context) {
    ...
    if req.Brand != nil { m["brand"] = *req.Brand }
    if req.State != nil { m["state"] = *req.State }
    if err := h.svc.Patch(c, id, m); err != nil { httpError(c, err); return }
    c.Status(http.StatusNoContent)
}
...
func (h *DeviceHandler) Delete(c *gin.Context) {
    id, ok := parseID(c)
    if !ok { return }
    if err := h.svc.Delete(c, id); err != nil { httpError(c, err); return }
    c.Status(http.StatusNoContent)
}
...
func parseID(c *gin.Context) (int64, bool) {
    ...
    if err != nil { c.String(http.StatusBadRequest, "invalid id"); return 0, false }
    return id, true
}
